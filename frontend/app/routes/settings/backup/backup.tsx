...
export function BackupSettings({ config, setNewConfig }: BackupSettingsProps) {
  ...
  const [restorePhase, setRestorePhase] = useState<"idle" | "staging" | "restarting" | "migrating">(
    "idle",
  );
  ...
  const updateBackup = useCallback(
    async (id: string, patch: { preserved?: boolean; notes?: string }) => {
      const form = new FormData();
      form.append("id", id);
  ...
  const deleteBackup = useCallback(
    async (id: string) => {
      ...
      try {
        const form = new FormData();
        form.append("id", id);
        const res = await fetch(withUrlBase("/api/db-backup-delete"), {
          method: "POST",
          body: form,
        });
        if (!res.ok) {
  ...
  const uploadBackup = useCallback(
    async (file: File) => {
      ...
      try {
        ...
        const res = await fetch(withUrlBase("/api/db-backup-upload"), {
          method: "POST",
          body: form,
        });
        const data = (await res.json().catch(() => null)) as BackupErrorResponse | null;
  ...
  return (
    <SettingsPage>
      <SettingsIntro>
        Create logical SQL dumps of your databases, schedule automatic backups, and restore from a
        previous snapshot when needed.
      </SettingsIntro>
              ...
              <div className="flex items-start gap-2 rounded-lg bg-base-200/30 px-3 py-2.5 text-xs leading-relaxed text-base-content/55">
                ...
                <p>
                  <span className="font-medium text-base-content/70">
                    Preserved backups are never pruned.
                  </span>{" "}
      ...
      <ConfirmModal
        ...
        message={
          <>
            Delete backup <span className="font-mono">{confirmDeleteId}</span>? This cannot be
            undone.
          </>
        }
        cancelText="Cancel"
      ...
      <ConfirmModal
        ...
        message={
          mainDatabaseProvider === "postgres" ? (
            ...
            <>
              Restoring <span className="font-mono">{confirmRestoreId}</span> replaces all settings,
              queue, history, and the WebDAV file tree. A pre-restore safety backup is created
              automatically. The server will restart into maintenance mode to apply the swap.
            </>
          )
        }
        checkboxMessage={
...
export function isBackupSettingsUpdated(
  config: Record<string, string>,
